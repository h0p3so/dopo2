...
public class OppVsIceView extends BiPanel
{
    private static OppVsIceView INSTANCE;
    ...
    private void displayFlavourButtons (final JPanel panel) {
        final Color [] colors = {
            ...
            Colors.CHARACTER_TROLL,
            Colors.CHARACTER_NARWHAL,
            Colors.CHARACTER_MACETA
        };
        ...
        final ButtonInfo [] info = {
            ...
            new ButtonInfo("2. Troll", ViewsId.SELECT_LEVEL, 3),
            new ButtonInfo("2. Narwhal", ViewsId.SELECT_LEVEL, 4),
            new ButtonInfo("2. Maceta", ViewsId.SELECT_LEVEL, 5),
        };
        ...
        for (int i = 0; i < info.length; i++) {
            ...
            Generics.styleChangeTextOnHover(this.flavorButtons[i], context.getName(), shortnames[context.getPosition()]);

            this.flavorButtons[i].setPreferredSize(new Dimension(90, 60));
            this.flavorButtons[i].setBackground(colors[context.getPosition()]);
        ...
        for (int i = 0; i < info.length; i++) {
            ...
            if (i < 3) {
                currentButton.addActionListener(e -> {
                    ...
                    if (p1ready && p2ready) {
                        INTER.getController().setGameMode(GameMode.OPP_VS_ICE);
                ...
                currentButton.addActionListener(e -> {
                    INTER.getController().setCharTypeTwo(types[context.getPosition()]);
                    p2ready = true;
     ...
     */
    public static OppVsIceView getInstance (final Intermediary inter) {
        if (INSTANCE == null) {
            INTER = inter;
            INSTANCE = new OppVsIceView();
        }
